...
def general_dc_roll(dc_name, num_of_dice=1, num_of_sides=20, total_mod=0):
    ...
    if auto_roll:
        ...
        base_dc_roll = input('What did you roll? ')
...
def attack_roll(total_attack_bonus=0, range_penalty=0):
    ...
    if auto_roll:
        ...
        print("%sRoll now. (Don't add in any mods)" % colorz.GREEN)
        base_attack_roll = int(input('What did you roll? '))
    ...
    if base_attack_roll == 1:
        print("Critical Failure!" % (colorz.RED, colorz.BLUE))
...
def damage_roll(num_of_dice=1, num_of_sides=6, total_mod=0, multiplier=1, damage_doubling=1):
    ...
    if auto_roll:
        ...
        print("%sRoll now. (Don't add in any mods)" % colorz.GREEN)
        total_damage = input('What did you roll? ')
...
def shield_attack(item_mod=0, charging=False, power_attack=0, cleave=False):
    ...
    hits = int(input('How many things were hit? '))
    if hits == 0:
        if cleave:
    ...
    for target in range(1, hits + 1):
        ...
        if charging:
            ...
            if can_trip.lower().startswith('n'):
                ...
                if touch_success.lower().startswith('y'):
                    ...
                    tripped = input('Did you trip it? (y|n) ')
                    ...
                    if tripped.lower().startswith('y'):
                        ...
                        if hit.lower().startswith('y'):
                            ...
                            input("Press Enter to continue..." + colorz.GREEN)
        ...
        print(colorz.BLUE)
        opposing_bull_rush_check = int(input("Opposing bull rush check? "))
        ...
        if knockback_distance > 0:
            hit = input(colorz.GREEN + "Did target hit a wall/solid object? (y|n) ")
            if hit.lower().startswith('y'):
...
def gore_attack(charging=False, power_attack=0, cleave=False):
    ...
    print(colorz.YELLOW)
    cleave = input("Did it cleave? (y|n) ")
    if cleave.lower().startswith('y'):
        print("%sCleaving....\n%s" % (colorz.PURPLE, colorz.GREEN))
...
while True:
    print("\n%s----Combat round #%d----%s" % (colorz.BLUE, round_num, colorz.YELLOW))
    #Auto roll?
    auto_roll = input('Auto roll dice?(y|n) ')
    ...
    #Charging?
    charging = input('Are you charging? (y|n) ')
    ...
    #Power attack?
    power_attack = int(input('How many points to power attack? (Max %d) '
        % base_attack_bonus))
    ...
    if total_damage:
        ...
        if 'shield' in total_damage:
            print("-Shield:")
            for target in list(total_damage['shield'].keys()):
    ...
    if cleave_damage:
        ...
        if 'shield' in cleave_damage:
            print("-Shield:")
            for target in list(cleave_damage['shield'].keys()):
